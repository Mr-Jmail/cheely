...
var slotMachine = function (el, options, track) {
    ...
    slot.startSpin = function () {
        slot.$el
            ...
            .animate({'top': '0'}, slot.spinSpeed*(1 + slot.loopCount/7), 'linear', function () {
                slot.lowerSpeed();
            });
    };
    ...
    slot.endSpin = function () {
        ...
        finalSpeed = ((slot.spinSpeed * 0.25) * (slot.liCount)) / slot.options.endNum;

        //slot.spinSpeed = slot.options.time / slot.options.loops;

        slot.$el
            .css('top', -slot.listHeight )
            .animate({'top': finalPos-100}, slot.spinSpeed*(1 + slot.loopCount/7) , slot.options.easing, function () { //finalSpeed
